...
private:
    ...
    BoardAnalysis analyzeBoard() const {
        ...
        // Sütun yükseklikleri: yukarıdan tara, ilk dolu hücre = yükseklik başlangıcı
        ...
        // Kapalı boşluklar: dolu bir bloğun altındaki boş hücreler
        a.holes = 0;
        for (int c = 0; c < BOARD_COLS; ++c) {
    ...
    {
        // Parçanın genişlik ve yüksekliğini hesapla
        Piece tmp;
        tmp.type     = type;
        ...
        // Kural 1: Düz tahta (bumpiness ≤ 4) → geniş yatay parçalar
        if (a.bumpiness <= 4) {
            if (type == TetrominoType::I && rotation % 2 == 0) score += 40.f;
            if (type == TetrominoType::O)                       score += 20.f;
        }

        // Kural 2: Engebeli tahta (bumpiness > 6) → T, S, Z
        if (a.bumpiness > 6) {
        ...
        // Kural 6: Kritik yükseklikte (board %60+ dolu) → küçük parçalar
    ...
    Piece spawnPieceAware(float quality) {
        ...
        // Tüm tip × döndürme kombinasyonlarını skora göre değerlendir
        ...
        // quality oranında en iyi parçayı, kalan oranda rastgele seç
        ...
        if (chance(m_rng) < quality) {
            ...
            // Rastgele dönüş (orijinal spawnPiece davranışını koru)
    ...
    // Tamamen rastgele parça — orijinal davranış (falling piece + slot 2)
    ...
    void spawnFallingPiece() {
        ...
        // Orijinal davranış: rastgele kolona düş
