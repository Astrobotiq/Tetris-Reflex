...
namespace engine {
    ...
    class AssetManager {
    public:
        // C#'taki Dictionary<string, Font> karþýlýðý
        sf::Font& getFont(const std::string& path) {
            // Font daha önce yüklendiyse direkt onu döndür
            if (auto it = m_fonts.find(path); it != m_fonts.end()) {
                return it->second;
            }

            // Yoksa yeni bir font oluþtur, yükle ve haritaya (map) kaydet
            sf::Font font;
            if (!font.openFromFile(path)) { // SFML 3'te fontlar openFromFile kullanýr
                std::cerr << "HATA: Font yuklenemedi -> " << path << '\n';
            }

            // std::move ile kopyalama yapmadan sahipliði map'e devrediyoruz
